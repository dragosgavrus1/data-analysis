...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = df.groupby(['native-country'])['salary'].apply(lambda x: (x == '>50K').sum() / len(x) * 100).idxmax()
    highest_earning_country_percentage = round(df.groupby(['native-country'])['salary'].apply(lambda x: (x == '>50K').sum() / len(x) * 100).max(), 1)
